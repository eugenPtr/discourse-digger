...
def batch_insert_posts(db: Prisma, posts: List[Dict[str, Any]], dao: Dao) -> int:
    ...
    modified_rows_count = db.post.create_many(data=data_to_insert, skip_duplicates=True)
    ...
    try:
        ...
        for dao in daos:
            ...
            posts = fetch_discourse_posts(
                dao.discourseApiKey, dao.discourseUsername, dao.apiBaseUrl, None
            )
            ...
            while no_data_count < 10:
                ...
                if len(posts) > 0:
                    no_data_count = 0
                    batch_insert_posts(db, posts, dao)
